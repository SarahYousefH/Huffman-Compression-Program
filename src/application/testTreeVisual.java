...
public class testTreeVisual extends Application {
	...
	public void start(Stage primaryStage) {
		...
		hBox.getChildren().addAll(new Label("Enter a text: "), tfText, btHuffmanTree);

		TextField tfBitString = new TextField();
		Button btDecodeToText = new Button("Decode to Text");
		HBox hBox2 = new HBox(5);
		hBox2.getChildren().addAll(new Label("Enter a bit string: "), tfBitString, btDecodeToText);
		...
		btHuffmanTree.setOnAction(e -> {
			...
			if (text.length() == 0) {
				...
				lblStatus.setText(text + " is encoded to " + encode(text, codes));
			}
		});
	...
	class TreeView extends Pane {
		...
		private void displayTree(Tree.Node root, double x, double y, double hGap) {
